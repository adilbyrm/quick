                        ...
                        @endif
                            ...
                            <div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" data-widget-editbutton="false">
                                ...
                                <header>
                                    ...
                                    <div style="margin:6px"><a href="{{ route('admin-ozellik-gruplari') }}" class="txt-color-blueLight">{{ $specGroup->specifics_group_name }}</a> - Özellik Adları</div>
                                    ...
                                    <div class="widget-body no-padding">
                                        ...
                                        <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                                            ...
                                            <tr>
                                                ...
                                                <th data-class="expand"><i class="text-muted hidden-md hidden-sm hidden-xs"></i> Özellik Değerleri</th>
                                                <th>Durum</th>
                                            ...
                                            @foreach($specNames as $spec)
                                                ...
                                                <tr>
                                                    <td>{{ $i }}</td>
                                                    <td>{{ $spec->specifics_name_order }}</td>
                                                    <td>{{ $spec->specifics_name }}</td>
                                                    <td><a href="{{ route('admin-ozellik-degerleri', $spec->id) }}">Özellik Değerleri</a></td>
                                                    <td>
                                                        <div class="smart-form">
                                                            <label class="toggle">
                                                                <input type="checkbox" name="" onchange="statusChange(this, '{{ $spec->id }}', 'specificsNames')" {{ $spec->specifics_name_status=='1' ? 'checked="checked"' : '' }}>
                                                                <i style="left:0" data-swchon-text="Aktif" data-swchoff-text="Pasif"></i>
                                                            </label>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <a href="javascript:void(0)" onclick="editSpecName('{{ $spec->id }}')"  rel="tooltip" data-placement="top" data-original-title="Düzenle" class="btn btn-labeled btn-primary"> <i class="glyphicon glyphicon-edit"></i> </a>
                                                        <a href="javascript:void(0)"
                                                           onclick="warning('{{ route("admin-ozellik-adi-sil", [$spec->id, "_token" => csrf_token()]) }}', 'Özellik Adını silmek istediğinize emin misiniz? Varsa alt özellikler de silinecektir.')"
                                                           rel="tooltip" data-placement="top" data-original-title="Özellik Adını Sil"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Yeni Özellik Ekle</h4>
                </div>
                <div class="modal-body">
                    <form method="post" action="{{ route('admin-ozellik-adi-ekle-p', $specGroup->id) }}" id="form" class="form-horizontal">
                        <div class="row">
                            <div class="col-md-12">
...
@section('footer_jscript')
    <script type="text/javascript">
        ...
        function editSpecName(ID) {

            $.post('{{ route("admin-ozellik-adi-duzenle") }}', {ID:ID}, function(resp) {
                if(!resp.length){
                    alert();
                    return false;
                }
